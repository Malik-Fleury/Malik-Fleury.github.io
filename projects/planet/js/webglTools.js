...
function initPrograms(shaderNamesList) {

    programsDict = new Object()

    for(let i = 0;i < shaderNamesList.length;i++)
    {
        let shaderName = shaderNamesList[i];
        var fgShader = getShader(glContext, shaderName + "-fs");
        var vxShader = getShader(glContext, shaderName + "-vs");
...
function getVertexBufferWithVertices(vertices) {
    ...
    glContext.bufferData(glContext.ARRAY_BUFFER, new Float32Array(vertices), glContext.DYNAMIC_DRAW);
